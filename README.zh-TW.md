...
`MultiPassInterpreter(..., prefer_ekg_group_coverage=True)` 可選擇把兩次裁切
分配給可見的肢體及胸前導程組，同時驗證原有疑點、發現新疑點並映回原圖。
必須在任何版面補正前已有完整、有效且明確標記的 12 導程，兩組裁切均有界，
且允許至少兩個系統性探查。急迫優先、外部波形／本機線索、缺失／未知導程、
其他模態與其他預算維持既有路徑。預設關閉，不增加模型呼叫；規劃涵蓋範圍
不代表診斷完整或正確，期限仍可能使某個已規劃裁切未執行。
